"""Benchmark six Python optimization frameworks on a shared supply LP.

Install optional solvers as needed:
  pip install pandas numpy psutil matplotlib pyoframe[highs] pyoptinterface[highs] pulp pyomo cvxpy mip
"""
...
try:
    ...
    Set = SolverFactory = Var = pyomo_minimize = value = None
...
def bench(label, fn):
    ...
    try:
        ...
        status, note = "skip", f"ImportError: {e}"
...
def solve_mip(data):
    if Model is None:
        raise ImportError("mip not installed")
    months, tanks, capacity, init_inv, dem, rout, costs, smax, sources = data
    ...
    for t in tanks:
        for mo in M:
            arrivals = xsum(float(rout.loc[s, t]) * Order[s, mo] for s in S)
...
def solve_pulp(data):
    ...
    for t in tanks:
        for mo in M:
            arrivals = pulp.lpSum(float(rout.loc[s, t]) * Order[(s, mo)] for s in S)
...
def solve_pyomo(data):
    if ConcreteModel is None:
        raise ImportError("pyomo not installed")
    months, tanks, capacity, init_inv, dem, rout, costs, smax, sources = data
    m = ConcreteModel()
    S = list(sources["source"])
    M = months
    ...
    routing = {(s, t): float(rout.loc[s, t]) for s in S for t in tanks}
    ...
    m.OBJ = Objective(rule=obj_rule, sense=pyomo_minimize)

    def nn_rule(m, t, mo):
        arr = sum(routing[s, t] * m.Order[s, mo] for s in m.S)
